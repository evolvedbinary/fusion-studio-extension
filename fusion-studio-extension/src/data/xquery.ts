export const XQUERY_CONFIG = {
  folding: {
    comments: {
      blockComment: ['(:', ':)'],
    },
    markers: {
      start: new RegExp('(declare|.*\\{\\s*(//.*)?$)'),
      end: new RegExp('(.*\\}\\s*;?\\s*|.*;)'),
    },
  }
};
export const XQUERY_PLIST = `<?xml version="1.0" encoding="UTF-8"?>
	...
	<array>
		...
		<dict>
			...
			<dict>
				...
				<dict>
					<key>name</key>
					<string>punctuation.section.scope.begin.xquery</string>
				</dict>
			</dict>
			<key>end</key>
			<string>(\\))</string>
			...
			<dict>
				...
				<dict>
					<key>name</key>
					<string>punctuation.section.scope.end.xquery</string>
				</dict>
			</dict>
			<key>name</key>
			<string>meta.scope.xquery</string>
			<key>patterns</key>
			<array>
				<dict>
					<key>include</key>
